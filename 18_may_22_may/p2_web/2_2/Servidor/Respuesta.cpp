...
void Respuesta::sendReplyBroadcast(char* respuesta) {
    struct mensaje msj_enviar;

    msj_enviar.messageType = 1;
    msj_enviar.requestId = 0;
    bzero(msj_enviar.arguments, TAM_MAX_DATA);
    memcpy(msj_enviar.arguments, respuesta, sizeof(int));
    ...
    int res;
    memcpy(&res, msj_enviar.arguments, sizeof(int));
    // printf("\t\tRespuesta enviada: %d.", res);
}